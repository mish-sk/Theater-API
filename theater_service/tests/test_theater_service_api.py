...
def sample_performance(**params):
    theater_hall = TheaterHall.objects.create(
        name="Blue", rows=20, seat_in_the_row=20
    )

    defaults = {
        "show_time": "2024-05-05 14:00:00",
        "play": None,
...
class AuthenticatedPlayApiTests(TestCase):
    ...
    def test_plays_list(self):
        sample_play(title="Test1")
        sample_play(title="Test2")
...
class AdminPlayApiTests(TestCase):
    ...
    def test_create_play_with_genres(self):
        ...
        payload = {
            ...
            "description": "Classic tale of power...",
        }
        res = self.client.post(PLAY_URL, payload)
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
    ...
    def test_create_play_with_actors(self):
        actor1 = Actor.objects.create(first_name="Liam", last_name="McPoyle")
        actor2 = Actor.objects.create(first_name="Janeth", last_name="Black")
        payload = {
            "title": "Macbeth",
...
class PlayImageUploadTests(TestCase):
    ...
    def test_put_play_not_allowed(self):
        ...
        play = sample_play(title="Test Play")
        url = detail_url(play.id)
